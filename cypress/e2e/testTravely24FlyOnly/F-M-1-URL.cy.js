...
describe('FLYONLU MOBILE', () => {
	...
	it('visit travely24', () => {
		cy.viewport(525, 425);
		cy.visit(customURL({}), { timeout: 15000 });
		cy.wait(4000);
	});
	context('TEST CHECK', () => {
		it('Test checked fly', () => {
	...
	context('TEST TEXT', () => {
		it('Test from to', () => {
			...
			cy.get('[data-cy="firstAirNameFrom"]', { timeout: 15000 }).then(textFrom => {
  ...
  it('- Test from/to in site text', () => {
    ...
    cy.get('[data-cy="firstArrivalAir"]').then(textTo => {
      ...
      console.log('to',to);
      expect(to).to.have.string(FD_URL_TO)
    })
  });
