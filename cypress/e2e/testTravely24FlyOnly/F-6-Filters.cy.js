...
describe('Test #3-3-1-Filters air test --- 08.10.2022', () => {
  ...
  context(`Search filters air`, () => {
    ...
    it('Check - Direktflug, test check and mini test APIes', () => {
      cy.viewport(1600, 1600);
      cy.server();
      cy.route('GET', customApiProductType({ directness: true })).as(
      ...
      cy.wait(`@getProductTypeDirektflug`, { timeout: 100000 })
        .its('status')
        .should('eq', 200);
    });
    ...
    it('Check - Business, test check and test APIes', () => {
      ...
      cy.wait(`@getProductTypeBusiness`, { timeout: 100000 })
        .its('status')
        .should('eq', 200);
      cy.wait(5000);
    });
    ...
    it('Return flight enter max time 22:40 and max test APIes', () => {
      ...
      sizeFetchTypeAER.forEach((_item, i) => {
        cy.wait(`@getAER_Price_${i + 1}`, { timeout: 100000 })
          .its('status')
          .should('eq', 200);
      });
      cy.wait(`@getProductTypePrice`, { timeout: 100000 })
        .its('status')
        .should('eq', 200);
    });
  });
