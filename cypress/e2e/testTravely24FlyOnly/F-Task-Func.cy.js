...
const AERprocessFareModel = (e, prevOfferCode = null) => {
	let totPrice = 0;
...
// доп функции которые вызываются внутри главной
const AERlegToTraffics = (l, type, time) => {
	return {
...
function humanHours(v) {
	return ('0' + v).slice(-2);
}
function convertMinsToHrsMins(_minutes) {
...
const processPartOfFlights = (response, type) => {
	let aerOffers = [];
...
describe('FUNC', () => {
	...
	it('Catch Api AER and PRODUCT', () => {
		...
		cy.wait('@ProductAPI', { timeout: 60000 }).then(list => {
			...
			const offerList = list.response.body.offerList;
		...
		cy.wait(['@AER_1', '@AER_2', '@AER_3', '@AER_4', '@AER_5'], { timeout: 60000 }).spread(
			(AER_1, AER_2, AER_3, AER_4, AER_5) => {
				...
				console.log('final', final);
					...
					cy.get('[data-cy="btnShowMoreFlight"]',{timeout:50000}).click({force:true})

				console.log('aer_1',aer_1);
							aer_1.forEach( e => {
							...
							aer_2.forEach( e => {
									 ...
									 console.log('partOfList2',partOfList);
							aer_3.forEach( e => {
										 const traf = AERprocessFareModel(e);
												 return  partOfList.push(...traf)
									 })
							...
							aer_5.forEach( e => {
								...
								console.log(`compareFlightPrices(final , partOfList , 'traffics2aer') ` , compareFlightPrices(final , partOfList , 'traffics2aer') );
								...
								final = compareFlightPrices(final , partOfList , 'traffics2aer')
								 ...
								 const urlFlight = 'PC5001|PC5678|'
								 const currentFlight = final.find(el => {
									console.log('getFlightByPackage(el)',getFlightByPackage(el),'urlFlight',urlFlight );
										 if(getFlightByPackage(el) == urlFlight) {
											 return el
										 }
									 })
					...
					// 	 cy.wait('@AER_1',{timeout:60000}).then( list => {
					// 		 let partOfList = []
					...
					//  })
					// 	 cy.wait('@AER_2',{timeout:60000}).then( list => {
					// 		 let partOfList = []
					// 		 const a = JSON.parse(list.response.body.trim())
					...
					//  })
					// 	 cy.wait('@AER_4',{timeout:60000}).then( list => {
					// 		let partOfList = []
					// 		 const a = JSON.parse(list.response.body.trim())
					...
					// 	 final = compareFlightPrices(final , partOfList , 'traffics2aer')
					//  })
					// 	 cy.wait('@AER_5',{timeout:60000}).then( list => {
					// 		 let partOfList = []
					// 		 const a = JSON.parse(list.response.body.trim())
